<?php
/*顺便做一下广告哈。关注：微信公众平台帐号--中国笑吧--随时随地看笑话*/
//header('content-type:text/html;charset=utf8');
//没有验证码的方法！不知道腾讯会不会把验证码搞出来
/* $arr = array(
	'account' => '公众平台帐号',
	'password' => '密码'
);
...
class Weixin {
	...
	//群发消息
    public function send($content='',$userId='') {
	...
	private function getUserFakeid(){
		$this->referer = 'http://mp.weixin.qq.com/cgi-bin/indexpage?t=wxm-index&lang=zh_CN';
		$this->getHeader = 0;
		$url = 'https://mp.weixin.qq.com/cgi-bin/contactmanagepage?t=wxm-friend&token='.$this->token.'&lang=zh_CN&pagesize='.$this->pageSize.'&pageidx=0&type=0&groupid=0';
		$user = $this->curlPost($url);
		$preg = "/id: '(\d+)'/";
		preg_match_all($preg,$user,$b);
		$i = 0;
		foreach($b[1] as $v){
			$this->getHeader = 0;
			$user = $this->curlPost('https://mp.weixin.qq.com/cgi-bin/contactmanagepage?t=wxm-friend&token='.$this->token.'&lang=zh_CN&pagesize='.$this->pageSize.'&pageidx=0&type=0&groupid='.$v);
			$preg = "/\"fakeId\"\\s\\:\\s\"(\\d+)\"/";
